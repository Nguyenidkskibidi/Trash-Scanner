...
// 🔥 FIX 1: LOẠI BỎ IMPORT COMPONENT TSX BỊ LỖI VÀ THAY BẰNG IMPORT ẢNH TĨNH (.png)
...
interface SetupScreenProps {
  ...
  t: (key: string, options?: Record<string, string | number>) => string; // Thêm prop t
}
...
const FemaleIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinecap="round" strokeLinejoin="round" {...props}><circle cx="12" cy="12" r="4"/><path d="M12 16v6m-3-3h6"/></svg>
);
...
export const SetupScreen: React.FC<SetupScreenProps> = ({ onSetupComplete, initialLang, t }) => {
  ...
  const nextStep = useCallback(() => {
    setError(null);
    if (step === 1 && !language) {
      setError(t('setup.error.language'));
      return;
    }
