...
import VietnamFlagIcon from '/VietnamFlagIcon.png'; 
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleLanguageChange = (lang: Language) => {
    ...
    // Chuyển đổi giới tính và danh xưng khi chuyển ngôn ngữ (Việt <-> Anh)
  ...
  const handleReminderToggle = async () => {
    ...
    if (!currentSettings.enableReminder) {
        if (!('Notification' in window)) {
            // Không dùng alert() mà dùng custom UI cho thông báo
            console.error(t('settings.notifications.notSupported'));
            return;
        }
        ...
        if (Notification.permission === 'granted') {
            ...
            // Hiển thị thông báo bằng custom UI thay vì alert
             console.warn(t('settings.notifications.denied'));
        } else { // 'default' state
            try {
        ...
        <div className="flex-grow p-6 overflow-y-auto space-y-8">
            ...
            <div className="mt-4 flex items-center justify-between bg-background/50 p-3 rounded-lg">
             ...
             <div className="mt-4 flex items-center justify-between bg-background/50 p-3 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                    {/* Sử dụng img cho cờ Việt Nam */}
                    <button onClick={() => handleLanguageChange('vi')} className={`p-1 rounded-md ${currentSettings.language === 'vi' ? 'bg-brand-green/30 ring-2 ring-brand-green' : 'opacity-50 hover:opacity-100'}`}>
                        <img 
                            src="/VietnamFlagIcon.png" 
                            alt="Cờ Việt Nam" 
                            className="w-8 h-6 object-cover rounded-sm" 
                            // Fallback nếu ảnh không tải được
                            onError={(e) => { (e.target as HTMLImageElement).onerror = null; (e.target as HTMLImageElement).src="https://placehold.co/32x24/C8102E/FFFFFF?text=VN"; }} 
                        />
                    </button>
                    {/* Sử dụng img cho cờ Vương quốc Anh */}
                    <button onClick={() => handleLanguageChange('en')} className={`p-1 rounded-md ${currentSettings.language === 'en' ? 'bg-brand-green/30 ring-2 ring-brand-green' : 'opacity-50 hover:opacity-100'}`}>
                        <img 
                            src="/UKFlagIcon.png" 
                            alt="Cờ Vương quốc Anh" 
                            className="w-8 h-6 object-cover rounded-sm" 
                            // Fallback nếu ảnh không tải được
                            onError={(e) => { (e.target as HTMLImageElement).onerror = null; (e.target as HTMLImageElement).src="https://placehold.co/32x24/003C71/FFFFFF?text=UK"; }} 
                        />
                    </button>
                </div>
            </div>
          ...
          <section className="animate-slide-in-up" style={{ animationDelay: '300ms' }}>
            ...
            <div className="mt-4 bg-background/50 p-4 rounded-lg flex items-center justify-between">
                ...
                {/* Thay thế window.confirm bằng một custom modal/logic để tuân thủ quy tắc */}
                <button
                    onClick={() => { console.log("Show custom confirm modal for reset..."); onResetApp(); }} // Tạm thời gọi onResetApp() sau khi log.
